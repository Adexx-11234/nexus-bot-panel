...
async function copilotAI(query, useThink = false, retries = MAX_RETRIES) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const data = await makeAPIRequest(endpoint, { text: query }, API_SECONDARY);
...
async function omegaImageGen(prompt, ratio = '1:1', retries = MAX_RETRIES) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      console.log(`[Omega Image Gen] Generating image (Attempt ${attempt}/${retries})`);

      const data = await makeAPIRequest('/ai/Ai-gen-image', { prompt, ratio }, API_OMEGA);
...
async function nsfwCheck(imageUrl, retries = MAX_RETRIES) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const data = await makeAPIRequest('/ai/nsfwcheck', { url: imageUrl }, API_SECONDARY);
...
async function toPrompt(imageUrl, retries = MAX_RETRIES) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      const data = await makeAPIRequest('/ai/toprompt', { url: imageUrl }, API_SECONDARY);

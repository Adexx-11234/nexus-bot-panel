...
export async function video2webp(buffer) {
  try {
    ...
    // Create a fresh FormData with ALL parameters from the server response
    // This preserves the full animation instead of limiting frames
    const form3 = new FormData()
    for (const [key, value] of Object.entries(obj)) {
      // Include all server parameters to preserve full animation
    ...
    // Look for animated WebP in the conversion output
    // The correct download link is /save/ezgif-XXXX.webp, not the .webp.html page
    let webpElement = null
    
    // First, look for direct .webp links (not .webp.html)
    ...
    // For animated WebPs, do NOT re-encode with sharp as it destroys animation
    // Only resize if the buffer is too large
    if (webpBuffer.length > 512 * 1024) {
      console.warn("WebP file is large, attempting to optimize while preserving animation...")
      // Try to use sharp's animated webp support
      try {

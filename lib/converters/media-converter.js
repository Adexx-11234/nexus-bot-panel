...
export async function video2webp(buffer) {
  try {
    ...
    let webpElement = document2.querySelector('a[href*=".webp"]')
    if (!webpElement) {
      // Try finding in image tags
      const imgElement = document2.querySelector("div#output img.output")
    ...
    if (!webpElement) {
      // Fallback: look for any webp link in output
      const links = document2.querySelectorAll("div#output a")
      for (const link of links) {
        if (link.href.includes(".webp")) {
          webpElement = { src: link.href }
          break
        }
      }
    }

  ...
  async execute(sock, sessionId, args, m) {
    ...
    if (!this._cleanupInterval) {
      this._cleanupInterval = setInterval(() => {
        ...
        logger.debug("Antilink cache cleared (2-minute cleanup)")
    ...
    try {
      switch (action) {
        ...
        case "warn":
          ...
          const newLimit = Number.parseInt(args[1])
          ...
          await sock.sendMessage(
            ...
            {
              text:
                `✅ Warning limit set to ${newLimit} warnings before removal
...
> © 𝕹𝖊𝖝𝖚𝖘 𝕭𝖔𝖙`,
          ...
          await sock.sendMessage(
            groupJid,
            {
              text:
                ...
                "• `.antilink warn [number]` - Set warning limit\n" +
  ...
  async getWarningLimit(groupJid) {
    try {
      const result = await pool.query(`SELECT warning_limit FROM groups WHERE jid = $1`, [groupJid])
  ...
  async setWarningLimit(groupJid, limit) {
    try {
      ...
      await pool.query(`UPDATE groups SET warning_limit = $1, updated_at = CURRENT_TIMESTAMP WHERE jid = $2`, [
        limit,
        groupJid,
      ])
      return true
  ...
  async handleLinkDetection(sock, sessionId, m) {
    try {
      ...
      const cacheKey = `${m.chat}:${m.key.id}`
      this._linkCache.set(cacheKey, {
        links: detectedLinks,
        timestamp: Date.now(),
      })

      setTimeout(() => {
        this._linkCache.delete(cacheKey)
      }, 120000)
    } catch (error) {

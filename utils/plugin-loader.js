 ...
 * 
 * THREE BEHAVIORS:
 * A) Admin Commands (.antilink on, .grouponly on): ALL respond, ONLY ONE updates DB
 * B) Anti-Plugins (link detection, spam): ONLY ONE processes, others skip entirely
 * C) Game Commands: Lock start commands only, not individual moves
 */
class MessageDeduplicator {
  constructor() {
    this.processedMessages = new Map() // messageKey -> { actions: Set, timestamp, lockedBy }
    this.cleanupInterval = 120000 // 2 minutes
    
    // Start cleanup interval
    this.startCleanup()
    
    log.info('MessageDeduplicator initialized (cleanup: 2min)')
  }

  /**
   * Generate unique message key
   * Format: groupJid_messageId
   */
  generateKey(groupJid, messageId) {
    if (!groupJid || !messageId) {
      log.warn('Invalid key generation attempt:', { groupJid, messageId })
      return null
    }
  ...
  /**
   * Try to acquire lock for processing (Anti-Plugins & Game Starts)
   * Returns true if lock acquired, false if already locked
   */
  tryLockForProcessing(messageKey, sessionId, action) {
    if (!messageKey) {
      log.warn('tryLockForProcessing called with null messageKey')
      return false
    }
    ...
    if (entry.actions.has(action)) {
      log.debug(`Action '${action}' already processed for ${messageKey}`)
      return false
    }

    // Check if locked by another session
    if (entry.lockedBy && entry.lockedBy !== sessionId) {
      log.debug(`Message ${messageKey} locked by ${entry.lockedBy}, session ${sessionId} skipping`)
      return false
    }
    ...
    log.debug(`Lock acquired: ${messageKey} by ${sessionId} for action '${action}'`)
    return true
  }

  /**
   * Mark action as processed (call AFTER action completes)
   */
  markAsProcessed(messageKey, sessionId, action) {
    if (!messageKey) {
      log.warn('markAsProcessed called with null messageKey')
      return
    }
    ...
    log.debug(`Marked as processed: ${messageKey} - action '${action}' by ${sessionId}`)
  }

  /**
   * Check if action already processed (for DB updates in admin commands)
   */
  ...
  cleanup() {
    ...
    if (cleanedCount > 0) {
      log.info(`Cleaned up ${cleanedCount} old message entries`)
    }
  }
  ...
  getStats() {
    return {
      ...
      entries: Array.from(this.processedMessages.entries()).map(([key, entry]) => ({
        key,
...
class PluginLoader {
  constructor() {
    ...
    // MESSAGE DEDUPLICATION - Prevents multiple sessions from processing same message
